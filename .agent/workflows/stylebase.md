...
These are reference implementations included with StyleBase. Teams using third-party UI libraries (MUI, shadcn, Kendo, etc.) can skip these and still use StyleBase's tokens, themes, layers, utilities, and layout systems. Third-party component CSS should be loaded in the `plugins` layer.

All components live in `src/scss/components/`, use `@layer components`, and follow `app-c-{name}` naming.

### Button (`_button.scss`)

```html
<button class="app-c-btn app-c-btn-primary app-c-btn-md">Label</button>
<button class="app-c-btn app-c-btn-secondary app-c-btn-icon-md" aria-label="Icon">X</button>
```

- **Variants:** `app-c-btn-primary`, `app-c-btn-secondary`
- **Sizes:** `app-c-btn-sm`, `app-c-btn-md`, `app-c-btn-lg`
- **Icon buttons:** `app-c-btn-icon-sm`, `app-c-btn-icon-md`, `app-c-btn-icon-lg` (square, no padding)
- **States:** `:hover`, `:focus-visible`, `:disabled`
- **Tokens:** `--btn-primary-*`, `--btn-secondary-*` (bg, bg-hover, color, border, disabled-bg, disabled-border, focus-ring)

### Textfield (`_textfield.scss`)

```html
<input type="text" class="app-c-textfield app-c-textfield-md" placeholder="Input" />
<textarea class="app-c-textfield app-c-textarea" rows="3" placeholder="Textarea"></textarea>
```

- **Sizes (input only):** `app-c-textfield-sm`, `app-c-textfield-md`, `app-c-textfield-lg`
- **Textarea:** `app-c-textarea` (no size variants, single style)
- **Error:** add `app-c-textfield-error` (overrides border/focus to `--status-error`)
- **States:** `:hover`, `:focus-visible`, `:disabled`, `:read-only`
- **Tokens:** `--textfield-*` (bg, color, placeholder, border, border-hover, bg-disabled, border-disabled, color-disabled, focus-ring)
...
- Checkmark and indeterminate dash rendered via `svg-mask` on `::before`
- **Tokens:** `--checkbox-*` (bg, bg-checked, bg-checked-disabled, border, border-hover, border-checked, mark, focus-ring)
...
<label class="app-c-radio">
  ...
  <span class="app-c-radio-text">Option</span>
</label>
```

- **Single size** - no size variants
- **States:** `:hover`, `:focus-visible`, `:checked`, `:disabled`
- Dot rendered via `::before` (CSS circle, no SVG)
- **Tokens:** reuses `--checkbox-*` tokens
...
- **Single size** - track `36x20`, knob `16x16`
- **States:** `:hover`, `:focus-visible`, `:checked`, `:disabled`
- Knob slides via `inset-inline-start` transition (RTL-safe)
- **Tokens:** `--toggle-*` (track, track-hover, track-checked, track-checked-hover, track-disabled, track-checked-disabled, knob, focus-ring)
...
- **Variants:** (default = neutral), `app-c-badge-success`, `app-c-badge-error`, `app-c-badge-warning`, `app-c-badge-info`
- **Tokens:** `--badge-{variant}-bg`, `--badge-{variant}-color` for each variant
